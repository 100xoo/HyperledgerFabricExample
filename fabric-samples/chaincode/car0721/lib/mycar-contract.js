...
class MycarContract extends Contract {
    ...
    async retrieveHistory(ctx, key) {
        ...
        const allResults = [];
        var res = await iterator.next(); //1개 읽기
        while (!res.done) { //읽은 내용이 있다면
            if (res.value) {
                console.log(res);     // 변경 이력 구조 확인
                if (res.value.isDelete !== true) {
                    const buf = res.value.value; //buffer
                    const str = buf.toString();  //string
                    const obj = JSON.parse(str); //object 

                    // 변경했던 시간 구하기 (UTC기준 시간)
                    const milliseconds = (
                        res.value.timestamp.seconds.low +
                        ((res.value.timestamp.nanos / 1000000) / 1000)
                    ) * 1000;

                    // UTC기준으로 + 9시간   
                    const ts = new Date(milliseconds + (1000 * 60 * 60 * 9));
                    allResults.push({ KEY: res.value.key, VALUE: obj, TS: ts });
                }
            }
            res = await iterator.next(); //다음것 1개 읽기
        }
